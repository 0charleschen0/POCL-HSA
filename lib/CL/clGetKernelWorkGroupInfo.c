  ...
  switch (param_name)
...
#ifdef debug
        	printf("by ccchen, Not Implement Yet!!\n");
#endif
        	kernel->reqd_wg_size = (int*)malloc(3*sizeof(int));
        	kernel->reqd_wg_size[0] = 0, kernel->reqd_wg_size[1] = 0, kernel->reqd_wg_size[2] = 0;
        }
#if 0
    ...
    case CL_KERNEL_PRIVATE_MEM_SIZE:
    {
      ...
      if(device->type&CL_DEVICE_TYPE_GPU){
    	  POCL_RETURN_KERNEL_WG_INFO(cl_ulong, kernel->hsaCodeDescriptor->workitem_private_segment_byte_size);
      }else
#endif
       {POCL_ABORT_UNIMPLEMENTED("CL_KERNEL_PRIVATE_MEM_SIZE is not supported.");}
    }
    default:
